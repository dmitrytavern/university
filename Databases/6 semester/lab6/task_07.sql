...
DROP VIEW IF EXISTS count_of_using_materials_min;
GO

-- выбираем все записи из приведущего запроса
-- к каждой записи присоеденяем запись о минимальном использовании материала в определенном цеху
-- выбираем только те записи, где material_total равны
-- присоеденям остальные таблицы для вывода названий цех и материала

CREATE VIEW count_of_using_materials_min
AS
SELECT
...
INNER JOIN (
	SELECT 
		warehouse_id,
		MIN(material_total) as material_total
	FROM count_of_using_materials
	GROUP BY
		warehouse_id
...
SELECT * FROM count_of_using_materials_min;
GO
